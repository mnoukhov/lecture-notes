...
strcpy(buffer, argv[1]);
	...
	\begin{itemize}
		\item \lstinline|printf(buffer)| parse buffer for \lstinline|\%|s and use whatever is on the stack to process found format params
		\item \lstinline|printf(%s%s%s%s)| may crash your program
		\item \lstinline|printf(%x%x%x%x)| dumps parts of the stack
		\item \lstinline|%n| will write to an address on the stack
	\end{itemize}
